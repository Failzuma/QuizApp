...
const mapBlueprintSchema = z.object({
  ...
  // The nodes array now only contains positions
...
// The onSubmit will pass data conforming to the backend's expectation
interface AddMapModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: { mapIdentifier: string; title: string; nodes: { posX: number; posY: number }[] }) => void;
}
...
export function AddMapModal({ isOpen, onClose, onSubmit }: AddMapModalProps) {
  const {
    ...
    defaultValues: {
      title: '',
      mapIdentifier: '',
      nodes: [{ posX: 100, posY: 100 }], // Start with one node position
    },
  });
  ...
  const handleFormSubmit = (data: MapBlueprintFormData) => {
    // The data is already in the correct format for the API payload
    onSubmit(data);
  };
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6 pt-4">
          ...
          <div className="space-y-4">
            ...
            {fields.length < 10 && (
              <Button type="button" variant="outline" size="sm" onClick={() => append({ posX: 0, posY: 0 })}>
